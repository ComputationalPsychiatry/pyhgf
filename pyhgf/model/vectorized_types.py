...
class LayerState(NamedTuple):
    ...
    This represents the state of a volatile node layer with both
    value level (external) and volatility level (internal) variables.
    """
...
class LayerParams(NamedTuple):
    ...
    bias: Array  # Additive bias term (zeros = no bias)
    ...
    def create(
        ...
        use_bias :
            If True, initialise bias to small random values. If False, zeros (no bias).
...
class NetworkState(NamedTuple):
    ...
    This represents the full state of a vectorized deep network,
    including all layer states, inter-layer weights, and parameters.
    """
