...
def value_volatility_node_posterior_update(
    attributes: dict,
    edges: Edges,
    node_idx: int,
) -> dict:
    """Update both internal levels of the value-volatility node.
    ...
    attributes[node_idx]["temp"]["volatility_prediction_error"] = volatility_prediction_error

    # 3. UPDATE VOLATILITY LEVEL (implicit internal)
    # Update precision first
    precision_vol = posterior_update_precision_volatility_level(
        attributes, node_idx
    )
