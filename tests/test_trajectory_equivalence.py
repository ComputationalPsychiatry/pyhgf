...
def _build_vanilla_network():
    net = Network()
    for _ in range(2):
        net.add_nodes(kind="volatile-state", tonic_volatility=TONIC_VOL,
                      autoconnection_strength=0.0)
    for _ in range(4):
        net.add_nodes(kind="volatile-state",
                      value_children=([0, 1], [COUPLING, COUPLING]),
                      coupling_fn=(jnp.tanh,),
                      tonic_volatility=TONIC_VOL, autoconnection_strength=0.0)
    for _ in range(2):
        net.add_nodes(kind="volatile-state",
    ...
    def test_vdn_vs_vanilla_output_layer(self):
        ...
        for node_idx in [0, 1]:
            ...
            for metric in METRICS:
                ...
                vdn_val = np.array(getattr(vdn.trajectories.layers[layer_idx], metric)[:, pos])
                np.testing.assert_allclose(
                    van_val, vdn_val, atol=ATOL,
